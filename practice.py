...
def get_sum_zero_pairs(numbers):
    ...
    for number in positive_numbers:
        if (-number) in negative_numbers:
            if (-number, number) not in sum_zero_pairs:
                sum_zero_pairs[(-number, number)] = None

    return sum_zero_pairs.keys()
...
def top_chars(phrase):
    ...
    max_counts = max(char_counts.values())

    return sorted([char for char, counter in char_counts.items() if counter == max_counts])
